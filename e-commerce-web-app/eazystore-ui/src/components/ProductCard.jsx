...
const ProductCard = ({ product }) => {
  ...
  return (
    <Link to={`/products/${productId}`} state={{ product }}>
      <motion.div
        className={`rounded-md h-[500px] w-80 m-8 shadow-md overflow-hidden border-2 flex flex-col ${isDarkMode ? 'border-[#4b5563]' : 'border-[#f3f4f6]'
          }`}
        initial={false}
        whileHover={{
          scale: 1.03,
      ...
      >
        ...
        <div className={`flex-1 flex flex-col p-2 ${isDarkMode ? 'bg-[#19242D]' : 'bg-[#ffffff]'}`}>
          ...
          <div className="flex justify-between items-center p-2 gap-2 mt-auto mb-2">
            <Button className={`${isDarkMode ? 'text-white bg-[#2E3947]' : 'text-[#4a2dce] bg-[#beb9e6]'}`}>
              ${price ?? 0}
            </Button>
